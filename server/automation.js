...
const AUTO_REPAIR_LOG = path.join(__dirname, '../auto-repair.log');
...
async function runAutoRepair() {
  try {
    ...
    if (result.success || result.data) {
      ...
      const logEntry = `[${timestamp}] Repaired ${repairDetails.length} vessel(s) | Total: $${totalCost}\n`;
      const detailsLog = repairDetails.map(v => `  - ${v.name}: ${v.wear}% wear -> $${v.cost}`).join('\n') + '\n';

      // Log to file
      try {
        await fs.appendFile(AUTO_REPAIR_LOG, logEntry + detailsLog);
      } catch (logError) {
        console.error('[Backend Auto-Repair] Failed to write log:', logError);
      }
